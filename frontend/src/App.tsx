...
function formatApiError(data: unknown, fallback: string): string {
  ...
  if (Array.isArray(data)) {
    const lines = data.map((item) => formatApiError(item, fallback)).filter(Boolean);
  ...
  if (isRecord(data)) {
    ...
    if (detail) return formatApiError(detail, fallback);
    if (message) return formatApiError(message, fallback);
    if (error) return formatApiError(error, fallback);
    if (typeof msg === "string") return `${Array.isArray(loc) ? loc.join(".") : "field"}: ${msg}`;

    const knownFields = ["type", "code", "param", "reason", "status"].filter((field) => data[field]);
    if (knownFields.length) {
      return knownFields.map((field) => `${field}: ${String(data[field])}`).join("\n");
    }
...
function formatUnknownError(error: unknown, fallback: string): string {
  if (error instanceof TypeError && /fetch|network|failed/i.test(error.message)) return BACKEND_UNREACHABLE_MESSAGE;
  if (error instanceof Error) return error.message && error.message !== "[object Object]" ? error.message : fallback;
  return formatApiError(error, fallback);
}

function errorActionHint(error: string): string | null {
  const lower = error.toLowerCase();
  if (lower.includes("backend is unreachable")) return "Start the FastAPI backend, then refresh or retry the action.";
  if (lower.includes("openai api key")) return "Open Settings, save an OpenAI key, or set OPENAI_API_KEY in the backend environment.";
  if (lower.includes("youtube api key") || lower.includes("youtube_api_key")) return "Set YOUTUBE_API_KEY in the backend environment, restart uvicorn, then retry YouTube search.";
  if (lower.includes("body.") || lower.includes("validation")) return "Check the highlighted form values and try again.";
  return null;
}

function EmptyState({ title, text, action }: { title: string; text: string; action?: string }) {
  return <div className="empty-state"><span className="empty-state-title">{title}</span><span>{text}</span>{action ? <span className="empty-state-action">{action}</span> : null}</div>;
}
...
function App() {
  ...
  async function loadLatestReadinessCheck() {
    ...
    catch (e) { setError(formatUnknownError(e, "Could not load readiness check.")); }
  ...
  async function analyzeImport(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(null); setEnhancement(null); setImportAnalysis(null); setPlanSavedMessage(null); setLoading("Extracting plan");
  ...
  async function enhancePlan() {
    ...
    setError(null); setEnhancement(null); setPlanSavedMessage(null); setLoading("Enhancing plan");
  ...
  async function savePlan(plan: AIParsedPlan, approvalStatus: string) {
    ...
    try { const saved = await api<{ program: Program }>("/imports/workout-plan/save", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ parsed_plan: plan, approval_status: approvalStatus }) }); setSelectedProgramId(saved.program.id); await loadPrograms(); setPlanSavedMessage(`Saved "${saved.program.name}". Open Training to start a session or Programs to review versions.`); setActiveView("dashboard"); }
  ...
  function renderPlanSavedMessage() {
  ...
  function renderDashboard() {
    ...
    return <section className="program-workspace cockpit-view">
      <section className="program-list demo-callout"><div><p className="eyebrow">Investor demo story</p><h3>Training memory builds after each completed session.</h3><p>SetPilot turns messy workout plans into guided training, records what actually happened, and keeps reflection plus progression advice separate from the plan.</p></div></section>
      {isColdStart ? <section className="program-list cold-start-card"><div><p className="eyebrow">Try the Phase-1 MVP demo flow</p><h3>No program loaded yet</h3><p>Open Import, click Load sample workout plan, then edit or analyze it. Training starts after a program and workout day exist.</p><ol className="demo-steps"><li>Load sample plan</li><li>Analyze and save</li><li>Start session</li><li>Log sets</li><li>Finish and review</li></ol></div><div className="cold-start-actions"><button className="primary-button" type="button" onClick={() => setActiveView("import")}>Go to Import</button><button className="secondary-button" type="button" onClick={() => setActiveView("programs")}>Create manually</button><button className="secondary-button" type="button" onClick={() => setActiveView("profile")}>Add profile context</button></div></section> : null}
      ...
      <div className="dashboard-summary-grid">
        ...
        <section className="program-list latest-workout-card">
          ...
          {dashboardSummary?.latest_completed_session_id ? <><p><strong>{dashboardSummary.latest_program_name ?? "Unknown program"}</strong> - {dashboardSummary.latest_workout_day_name ?? "Unknown day"}</p><p>Finished: {dashboardSummary.latest_completed_session_finished_at ? new Date(dashboardSummary.latest_completed_session_finished_at).toLocaleString() : "not recorded"}</p>{dashboardSummary.latest_exercise_names.length ? <p>Exercises: {dashboardSummary.latest_exercise_names.join(", ")}</p> : <p>No exercise names were captured.</p>}<button className="secondary-button compact-button" type="button" onClick={() => void loadSessionDetail(dashboardSummary.latest_completed_session_id as number)}>View latest session detail</button></> : <p className="dashboard-empty-state">No sessions yet. Start a workout to create training history.</p>}
        </section>
        <section className="program-list dashboard-insight-card">
          <h3>Latest Reflection</h3>
          {dashboardSummary?.latest_reflection_summary ? <p>{dashboardSummary.latest_reflection_summary}</p> : <p className="dashboard-empty-state">No reflections yet. Finish a session, then generate one.</p>}
        </section>
        <section className="program-list dashboard-insight-card">
          <h3>Next Session Suggestions</h3>
          {dashboardSummary?.latest_progression_suggestions.length ? <div className="dashboard-insight-list">{dashboardSummary.latest_progression_suggestions.map((suggestion, index) => <p key={`${suggestion}-${index}`}>{suggestion}</p>)}</div> : <EmptyState title="No progression suggestions yet" text="Generate them after opening a completed session." action="Next: open a completed session." />}
        </section>
      </div>
      <div className="program-list dashboard-insight-card">
        <h3>Quick Actions</h3>
        <div className="quick-actions"><button className="primary-button" type="button" onClick={() => setActiveView("import")}>Import workout plan</button><button className="secondary-button" type="button" onClick={() => setActiveView("profile")}>Add profile</button><button className="secondary-button" type="button" onClick={() => setActiveView("readiness")}>Add readiness</button><button className="secondary-button" type="button" onClick={() => setActiveView("training")}>{activeSession?.status === "active" ? "Continue training" : "Start training"}</button><button className="secondary-button" type="button" onClick={() => setActiveView("programs")}>View programs</button><button className="secondary-button" type="button" onClick={() => setActiveView("settings")}>Open settings</button></div>
      </div>
      ...
      <div className="program-list recent-sessions-card">
        ...
        {recentSessions.length === 0 ? <EmptyState title="No sessions yet" text="Start a workout to create training history." action="Next: choose a program and workout day." /> : <div className="program-cards">{recentSessions.slice(0, 5).map((session) => {
      ...
      {selectedSessionDetail && selectedDetailSummary ? <div className="program-list session-detail-panel">
        ...
        <p className="context-note">Reflection and progression are advisory. The plan is never silently changed.</p>
        ...
        <div className="progression-panel">
          ...
          {!progressionLoading && selectedSessionDetail.status === "completed" && selectedSessionProgressionSuggestions.length === 0 ? <EmptyState title="No progression suggestions generated" text="Suggestions are created from completed set data and do not change your plan." action="Next: click Generate progression suggestions." /> : null}
        ...
        <div className="reflection-panel">
          ...
          {!reflectionLoading && selectedSessionDetail.status === "completed" && !selectedSessionReflection ? <EmptyState title="No AI reflection generated" text="Reflection appears after a completed session and requires an OpenAI key." action="Next: generate reflection or check Settings." /> : null}
  ...
  function renderTrainingViewV2() {
    ...
    return <section className="program-workspace">
      <div className="section-heading">
        <div><p className="eyebrow">Training cockpit</p><h2>Train From The Plan, Log Reality</h2></div>
        <p>{selectedProgram ? `Selected program: ${selectedProgram.name}` : "Select a program first. The cockpit guides planned work while you record actual reps, weight, difficulty, and rest."}</p>
      </div>
      ...
      <div className="active-session-cockpit">
        <div className="session-panel-header">
          ...
          <div className="card-actions">
            ...
            <button className="secondary-button finish-session-button" disabled={!sessionIsActive || loading !== null} type="button" onClick={() => void finishWorkoutSession()}>Finish Session</button>
          </div>
        </div>
        ...
        {!selectedProgram ? <EmptyState title="No program selected" text="Select a saved program or import a plan before starting a workout session." action="Next: open Import or Programs." /> : !selectedWorkoutDay ? <EmptyState title="No workout day selected" text="Choose the day you are training before starting the session." action="Next: select a workout day below." /> : null}
      ...
      <div className="program-grid training-tools-section"><form className="program-form" onSubmit={handlePlannedSetSubmit}><h3>{editingPlannedSetId ? "Edit planned set" : "Add planned set weight"}</h3><p className="muted">Selected exercise: {selectedExercise ? selectedExercise.movement_name : "none"}</p><div className="inline-fields"><label>Set number<input disabled={selectedExerciseId === null} type="number" value={plannedSetForm.set_number} onChange={(e) => setPlannedSetForm({ ...plannedSetForm, set_number: e.target.value })} /></label><label>Target reps<input disabled={selectedExerciseId === null} value={plannedSetForm.target_reps} onChange={(e) => setPlannedSetForm({ ...plannedSetForm, target_reps: e.target.value })} /></label></div><div className="inline-fields"><label>Suggested weight<input disabled={selectedExerciseId === null} list="weight-options" type="number" step="0.5" value={plannedSetForm.suggested_weight} onChange={(e) => setPlannedSetForm({ ...plannedSetForm, suggested_weight: e.target.value })} /><datalist id="weight-options"><option value="20" /><option value="30" /><option value="40" /><option value="50" /><option value="60" /><option value="80" /><option value="100" /></datalist></label><label>Unit<select disabled={selectedExerciseId === null} value={plannedSetForm.weight_unit} onChange={(e) => setPlannedSetForm({ ...plannedSetForm, weight_unit: e.target.value })}><option value="kg">kg</option><option value="lb">lb</option><option value="bodyweight">bodyweight</option></select></label></div><label>Note<textarea disabled={selectedExerciseId === null} value={plannedSetForm.note} onChange={(e) => setPlannedSetForm({ ...plannedSetForm, note: e.target.value })} /></label><button className="primary-button" disabled={selectedExerciseId === null} type="submit">{editingPlannedSetId ? "Save planned set" : "Add planned set"}</button></form><div className="program-list"><h3>Planned set weights</h3>{selectedExercise ? <p className="muted">For {selectedExercise.movement_name}</p> : <EmptyState title="No exercise selected" text="Select an exercise before adding planned set weights." />}{selectedExercise && plannedSets.length === 0 ? <EmptyState title="No planned set weights" text="Add optional set-level targets if you want weight guidance during training." action="Next: add a planned set or train without one." /> : null}<div className="program-cards">{plannedSets.map((set) => <article className="program-card" key={set.id}><div><h4>Set {set.set_number}</h4><p>{set.target_reps} reps · {set.suggested_weight ?? "—"} {set.weight_unit}</p><span>{set.note || "No note"}</span></div><div className="card-actions"><button className="secondary-button compact-button" onClick={() => { setEditingPlannedSetId(set.id); setPlannedSetForm({ set_number: String(set.set_number), target_reps: set.target_reps, suggested_weight: set.suggested_weight === null ? "" : String(set.suggested_weight), weight_unit: set.weight_unit, note: set.note }); }} type="button">Edit</button><button className="danger-button compact-button" onClick={() => void deletePlannedSet(set.id)} type="button">Delete</button></div></article>)}</div></div></div>
      <div className="program-grid training-tools-section"><form className="program-form" onSubmit={handleYouTubeSubmit}><h3>Add YouTube example</h3><p className="muted">Selected exercise: {selectedExercise ? selectedExercise.movement_name : "none"}</p><button className="secondary-button" disabled={selectedExerciseId === null || loading !== null} type="button" onClick={() => void findYouTubeExamples()}>Find YouTube examples automatically</button><label>YouTube link or video ID<input disabled={selectedExerciseId === null} placeholder="https://youtu.be/... or video ID" value={youtubeForm.input} onChange={(e) => setYoutubeForm({ ...youtubeForm, input: e.target.value })} /></label><label>Title<input disabled={selectedExerciseId === null} placeholder="Bench Press tutorial" value={youtubeForm.title} onChange={(e) => setYoutubeForm({ ...youtubeForm, title: e.target.value })} /></label><div className="inline-fields"><label>Channel<input disabled={selectedExerciseId === null} value={youtubeForm.channel_name} onChange={(e) => setYoutubeForm({ ...youtubeForm, channel_name: e.target.value })} /></label><label>Order<input disabled={selectedExerciseId === null} type="number" value={youtubeForm.display_order} onChange={(e) => setYoutubeForm({ ...youtubeForm, display_order: e.target.value })} /></label></div><label>Thumbnail URL optional<input disabled={selectedExerciseId === null} value={youtubeForm.thumbnail_url} onChange={(e) => setYoutubeForm({ ...youtubeForm, thumbnail_url: e.target.value })} /></label><button className="primary-button" disabled={selectedExerciseId === null} type="submit">Add video manually</button></form><div className="program-list"><h3>YouTube examples</h3>{selectedExercise ? <p className="muted">Stored videos for {selectedExercise.movement_name}</p> : <EmptyState title="No exercise selected" text="Select an exercise before attaching technique videos." />}{selectedExercise && youtubeVideos.length === 0 ? <EmptyState title="No videos attached" text="Add a known YouTube link or use search after YOUTUBE_API_KEY is configured." action="Next: add a video or configure YouTube search." /> : null}<div className="video-grid">{youtubeVideos.map((video) => <article className="video-card" key={video.id}><iframe className="video-frame" src={`https://www.youtube.com/embed/${video.youtube_video_id}`} title={video.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /><div className="video-meta"><h4>{video.title}</h4><p>{video.channel_name || "Unknown channel"}</p><button className="danger-button compact-button" onClick={() => void deleteYouTubeVideo(video.id)} type="button">Remove</button></div></article>)}</div></div></div>
    </section>;
  }

  function renderSettingsView() {
    return <section className="program-workspace"><div className="section-heading"><div><p className="eyebrow">Settings</p><h2>API Keys & Configuration</h2></div><p>Local demo setup can run without keys. Configure keys only when testing AI import, enhancement, reflection, or YouTube search.</p></div><div className="program-grid"><form className="program-form" onSubmit={saveOpenAIKey}><h3>OpenAI access</h3><p className="muted">Status: {openAIKeyStatus.configured ? `Configured from ${openAIKeyStatus.source} (${openAIKeyStatus.masked_key})` : "Not configured"}</p><p className="setup-note">OpenAI powers AI import, enhancement, session reflection, and future proposal workflows. Missing keys are expected in local demos unless configured. Never commit real API keys.</p><label>API key<input autoComplete="off" type="password" value={openAIKeyInput} onChange={(e) => setOpenAIKeyInput(e.target.value)} /></label><div className="form-actions"><button className="primary-button" disabled={!openAIKeyInput.trim() || loading !== null} type="submit">Save key</button><button className="secondary-button" disabled={loading !== null} type="button" onClick={() => void loadOpenAIKeyStatus()}>Check</button><button className="danger-button" disabled={loading !== null} type="button" onClick={() => void clearOpenAIKey()}>Clear</button></div></form><div className="program-list"><h3>YouTube key</h3><p>YouTube search uses backend environment variable <strong>YOUTUBE_API_KEY</strong>. Keep it in PowerShell/backend environment, not in GitHub.</p><p className="setup-note">Missing YouTube keys are normal in local demos. Manual video links still work. After changing environment variables, restart the backend so uvicorn can read them.</p></div></div></section>;
  }
  ...
  return <main className="app-shell">
    ...
    <section className="dashboard"><div className="intro"><h2>SetPilot</h2><p><strong>AI workout operating system for plan import, guided sessions, and progression memory.</strong></p><p>Paste a workout plan, turn it into a structured program, train inside the session cockpit, and let SetPilot reflect on what happened.</p></div><nav className="actions" aria-label="SetPilot navigation">{navItems.map((item) => <button className={`action-button${activeView === item.id ? " active-action" : ""}`} key={item.id} type="button" onClick={() => setActiveView(item.id)}>{item.label}<span>{item.subtitle}</span></button>)}</nav></section>
