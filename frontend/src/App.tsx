...
function App() {
  ...
  useEffect(() => { if (selectedProgramId === null) { setWorkoutDays([]); setSelectedWorkoutDayId(null); setExercises([]); setSelectedExerciseId(null); setPlannedSets([]); return; } void loadWorkoutDays(selectedProgramId).catch((e) => setError(e.message)); }, [selectedProgramId]);
  useEffect(() => { if (selectedProgramId === null || selectedWorkoutDayId === null) { setExercises([]); setSelectedExerciseId(null); setPlannedSets([]); return; } void loadExercises(selectedProgramId, selectedWorkoutDayId).catch((e) => setError(e.message)); }, [selectedProgramId, selectedWorkoutDayId]);
  useEffect(() => { if (selectedExerciseId === null) { setPlannedSets([]); return; } void loadPlannedSets(selectedExerciseId).catch((e) => setError(e.message)); }, [selectedExerciseId]);
  ...
  async function handleExerciseSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try { const saved = await api<WorkoutExercise>(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); setEditingExerciseId(null); setExerciseForm({ movement_name: "", sets: "3", reps: "8-10", rest_seconds: "90", notes: "", exercise_order: "1" }); setSelectedExerciseId(saved.id); await loadExercises(selectedProgramId, selectedWorkoutDayId); }
  ...
  return <main className="app-shell">
    <header className="top-bar"><h1 className="brand">SetPilot</h1><span className="phase-label">Extract → Enhance → Weight Plan</span></header>
    <section className="dashboard"><div className="intro"><h2>Paste chaos. Train with structure.</h2><p>Extract the plan, enhance it based on readiness/body context, then prepare set-level weights. Investor brain says: this is where the app stops being a note-taking toy.</p></div><div className="actions">{["API Key", "Extract", "Enhance", "Weights"].map((label) => <button className="action-button" key={label} type="button">{label}<span>Current workflow</span></button>)}</div></section>
