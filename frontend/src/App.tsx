...
type AuthStatus = { auth_required: boolean; supabase_url_configured: boolean; jwt_secret_configured: boolean; jwks_url_configured: boolean; verification_configured: boolean; verification_mode: string; local_fallback_enabled: boolean };
...
function App() {
    ...
    return <section className="program-workspace cockpit-view">
      ...
      <div className="dashboard-summary-grid">
        ...
        <section className="program-list dashboard-insight-card">
          ...
          {dashboardSummary?.latest_progression_suggestions.length ? <div className="dashboard-insight-list">{dashboardSummary.latest_progression_suggestions.map((suggestion, index) => <p key={`${suggestion}-${index}`}>{suggestion}</p>)}</div> : <EmptyState title="No progression suggestions yet" text="Finish a session, then generate suggestions from the session detail panel." action="Next: open a completed session." />}
        </section>
      </div>
      ...
      <div className="program-list recent-sessions-card">
        ...
        {recentSessions.length === 0 ? <EmptyState title="No sessions yet" text="Start one from the Training cockpit and completed sets will show here." action="Next: choose a program and workout day." /> : <div className="program-cards">{recentSessions.slice(0, 5).map((session) => {
  ...
  function renderAuthView() {
    ...
    return <section className="program-workspace"><div className="section-heading"><div><p className="eyebrow">Auth</p><h2>Supabase Auth Foundation</h2></div><p>Authentication is available for sign up, sign in, sign out, and session state. Signed-in data ownership is enabled through backend routes.</p></div>
      <div className="program-grid">
        ...
        <div className="program-list">
          ...
          <p><strong>Local fallback enabled:</strong> {backendAuthStatus?.local_fallback_enabled ? "true" : "false"}</p>
          <p className="setup-note">Programs, Sessions, Profile, Readiness, Import saves, YouTube saves, reflections, progression, and dashboard data are scoped by the backend current user. Local development uses `local-dev-user` when auth is not required.</p>
